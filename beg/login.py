...
class App:
	...
	def index(self):
		...
		self.cur.execute("select humidity,temperature from sensor order by date desc limit 1")
		sen = self.cur.fetchone()
		self.cur.execute("select status from switch where s_id = 1 order by time desc limit 1")
		s1 = self.cur.fetchone()
		self.cur.execute("select status from switch where s_id = 2 order by time desc limit 1")
